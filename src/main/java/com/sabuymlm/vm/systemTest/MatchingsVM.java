...
public class MatchingsVM extends CommonVM<MatchingDefineHeader> implements Serializable {
    ...
    public void init(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("icon") String icon, @ExecutionArgParam("headerLabel") String headerLabel) {
        initialVM("/secured/systemTest/addMatchingsDef.zul", "id", icon, headerLabel);
        search();
    ...
    @Override
    public void load() {
        items = systemTestService.findAllMatchingHeaders(getActivePage(), getPageSize(), order, getKeyword());
    }

    @Override
    public void deleteSelected() {
        systemTestService.deleteAllMatchings(selectItems);  
        search();
        BindUtils.postNotifyChange(null, null, MatchingsVM.this, ".");
    }
