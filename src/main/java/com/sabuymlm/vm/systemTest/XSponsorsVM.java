...
public class XSponsorsVM extends CommonVM<XSponsorDefineHeader> implements Serializable {
    ...
    public void init(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("icon") String icon, @ExecutionArgParam("headerLabel") String headerLabel) {
        initialVM("/secured/systemTest/addXSponsorsDef.zul", "id", icon, headerLabel);
        search();   
        headers.add(new String[]{"No.", "60px"});
        headers.add(new String[]{"แก้ไข", "60px"});
        headers.add(new String[]{"โบนัส(ตำแหน่ง)", "100px"});
        headers.add(new String[]{"โบนัส", "80px"});
    ...
    @Override
    public void load() {
        items = systemTestService.findAllXSponsorHeaders(getActivePage(), getPageSize(), order, getKeyword());
    }

    @Override
    public void deleteSelected() {
        systemTestService.deleteAllXSponsors(selectItems);  
        search();
        BindUtils.postNotifyChange(null, null, XSponsorsVM.this, ".");
    } 
